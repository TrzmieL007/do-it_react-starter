/**
 * Created by trzmiel007 on 12/12/16.
 */
...
class Image extends React.Component {
    ...
    progress(e){
        console.log('progress event: %o',e);
    }
    load(src){
        let done = file => {
            console.log('file data: %o',file);
        };
        let onProgress = this.progress.bind(this);
        $.ajax({ url : src, done, onProgress });
    }

    loaded(){
        this.end = +(new Date());
        console.log('loaded at %s, and that took %s',this.end,this.end-this.start);
        this.container.classList.remove('loading');
    }

    render() {
        this.start = +(new Date());
        console.log('render at %s',this.start);
        return <div ref={e => this.container = e} styleName="img" className="loading">
            <img src={this.props.src} ref={e => this.image = e} {...this.props.props} onLoad={this.loaded.bind(this)}/>
        </div>;
    }
}
...
export default CSSModules(Image,scss);
/** Uncoment this line for use with require(...) function **/
//module.exports = CSSModules(Image,scss);
